...
│   ├── repository.ts           # OrderRepository interface
...
│   │   └── port.ts             # PlaceOrderUseCase interface
│   ├── ship_order/
│   │   └── ...
│   └── get_order/
...
├── shared/
│   ├── unit_of_work.ts         # UnitOfWork interface
│   ├── event_publisher.ts      # EventPublisher interface
...
import { UnitOfWork } from '@/application/shared/unit_of_work';
import { EventPublisher } from '@/application/shared/event_publisher';
...
import { PlaceOrderUseCase } from '@/application/orders/place_order/port';
import { GetOrderUseCase } from '@/application/orders/get_order/port';
...
flowchart TB
    ...
    subgraph Infrastructure["Infrastructure"]
        ...
        RabbitMQ -.->|implements| EventPub["EventPublisher"]
    end
...
export function configureContainer(): Container {
  ...
  container.bind<ProductRepository>('ProductRepository').to(PostgresProductRepository);
  container.bind<UnitOfWork>('UnitOfWork').to(PostgresUnitOfWork);
  container.bind<EventPublisher>('EventPublisher').to(RabbitMQEventPublisher);
  container.bind<PlaceOrderUseCase>('PlaceOrderUseCase').to(PlaceOrderHandler);
