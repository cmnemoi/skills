...
import { PlaceOrderPort } from '@/application/ports/driver/place_order_port';
import { GetOrderPort } from '@/application/ports/driver/get_order_port';
...
export function createPlaceOrderCommand(placeOrder: PlaceOrderPort): Command {
...
|---------|------|---------|
| Interface/Impl | `OrderRepository` | `PostgresOrderRepository` |
| Port suffix | `OrderRepositoryPort` | `PostgresOrderAdapter` |
| Using prefix | `OrderStorage` | `OrderStorageUsingPostgres` |
...
function configureDevelopment(container: Container): void {
  ...
  container.bind<EventPublisherPort>('EventPublisherPort')
    .to(InMemoryEventPublisher);
  container.bind<PaymentGatewayPort>('PaymentGatewayPort')
